...
digraph process {
    ...
    "Setup: current directory, workspace + ledger, read plan + spec, pre-flight scan" [shape=box];
    ...
    "Setup: current directory, workspace + ledger, read plan + spec, pre-flight scan" -> "task-start: brief + BASE; read the brief";
...
Run the plan's final verification commands and report completion directly
to your human partner with:
...
Done. All tasks completed, verification passed, and final review is clean.
Rulings and deferred minors are reported above.
```
